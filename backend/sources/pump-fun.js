...
// Read full bonding curve state — used for gate 3 check (no API needed)
// Returns virtualSolReserves, realSolReserves, progress % and creator
async function readBondingCurveState(tokenMint) {
  try {
    const bondingCurve = findBondingCurvePDA(tokenMint);
    const accountInfo = await executeWithFallback(conn => conn.getAccountInfo(bondingCurve));
    ...
    const data = accountInfo.data;
    // Parse u64 values as little-endian BigInt
    const readU64 = (offset) => Number(data.readBigUInt64LE(offset));

    const virtualTokenReserves = readU64(8);
    const virtualSolReserves   = readU64(16);  // in lamports
    const realTokenReserves    = readU64(24);
    const realSolReserves      = readU64(32);  // in lamports
    const tokenTotalSupply     = readU64(40);
    const complete             = data[48] === 1;

    const solInCurve      = virtualSolReserves / 1e9;         // SOL
    const realSolInCurve  = realSolReserves / 1e9;            // SOL actually bought
    const GRADUATION_SOL  = 85;                                // ~85 SOL to graduate
    const progress        = Math.min(100, (realSolInCurve / GRADUATION_SOL) * 100);
    const creator         = parseCreatorFromBondingCurve(accountInfo);

    // Initial state: virtualSolReserves ~= 30 SOL (30_000_000_000 lamports)
    // Any real buying pushes realSolReserves above 0
    const hasRealActivity = realSolInCurve > 0.001; // at least 0.001 SOL bought

    return {
      virtualSolReserves: solInCurve,
      realSolReserves: realSolInCurve,
      virtualTokenReserves,
      realTokenReserves,
      progress,          // 0-100% to graduation
      complete,          // true = graduated to Raydium
      hasRealActivity,   // true = at least one real buy happened
      creator: creator?.toString() || null
    };
  } catch (err) {
    return null;
  }
...
module.exports = { pumpBuy, pumpSell, isOnBondingCurve, findBondingCurvePDA, readBondingCurveState };
